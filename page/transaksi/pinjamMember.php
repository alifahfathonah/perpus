...
<div class="row">
 ...
 <div class="panel-body">
       ...
       <?php
       $sql = $koneksi->query("SELECT * from user where status ='member' and id_user='".$_SESSION['member']."' order by id_user asc ");
       $data= $sql->fetch_assoc();
       ?>
       <input class="form-control hidden" name="peminjam" value="<?=$data[id_user]?>" readonly/>
       <input class="form-control" name="peminjam 2" value="<?=$data[nama]?>" readonly/>
             </div>
    ...
    echo "<option value='$data[id_buku]' > $data[judul_buku],  Stok : $data[stok]</option>";
    }
    ?>
    </select>
          </div>
  ...
  <table class="table table-striped table-bordered table-hover">
      ...
      if(isset($_POST['tambah'])){
         ...
         window.location.href="?page=pinjamMember";
         </script>
         <?php
          }
         }
 ...
 ob_start();
    ...
    $pinjam = $koneksi->query("INSERT INTO peminjaman( id_user, tgl_pinjam, limit_tanggal, status_peminjaman, terlambat, denda)
       VALUES ( '$tmppeminjam[$i]', '$tmptgl_pinjam[$i]', '$tmplimit_tanggal[$i]', 'onProses' , '0 Hari', '0' )");
